// Reglas de XP, niveles y condiciones de badges. Todo local.
...
export function checkBadgeCondition(
  ...
  switch (type) {
    case 'lessons_completed':   return stats.lessons_completed >= (count ?? 1)
    case 'chapters_completed':  return stats.chapters_completed >= (count ?? 1)
    case 'quiz_perfect_score':  return stats.quiz_perfect_scores >= (count ?? 1)
    case 'quizzes_completed':   return stats.quizzes_completed >= (count ?? 1)
    case 'exercises_passed':    return stats.exercises_passed >= (count ?? 1)
    case 'streak_days':         return stats.streak_days >= (count ?? 1)
    case 'course_completed':    return stats.courses_completed.includes(condition.course_id as string)
    case 'all_courses_completed': return stats.courses_completed.length >= 5
